...
void createRandomWormholeElement(Spiel *spiel_ptr)
{
	if((clock() - spiel_ptr->wormhole_ptr->last_time) >= spiel_ptr->wormhole_ptr->time_intervall)
	{
		...
		do
		{
			...
			wormhole->y1 = SPIELFELD_OFFSET_Y + 1 + rand() % (SPIELFELD_XSIZE - 1);
			wormhole->x2 = SPIELFELD_OFFSET_X + 1 + rand() % (SPIELFELD_XSIZE - 1);
			wormhole->y2 = SPIELFELD_OFFSET_Y + 1 + rand() % (SPIELFELD_XSIZE - 1);
		}while(wormhole->x1 != wormhole->x2 && wormhole->y1 != wormhole->y2);

		pushWormholeElement(spiel_ptr->wormhole_ptr->wormholeList_ptr->head, wormhole);
	}
}
